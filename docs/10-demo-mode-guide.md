...
## 六、生产环境的 Demo Mode(提交审核时)

本地 Demo 用的模拟录音机跑在开发机上,审核人员用的是自己的手机 + 我们的生产 API,
因此提交审核前必须准备:

1. **审核账号**:预置一个账号(邮箱走真实邮件服务),在 App Store Connect /
   Play Console 的审核信息里填写账号与验证码获取方式;
2. **App 内 Demo 入口**:审核账号登录后,设备页提供"使用演示设备"选项,
   由服务端返回一组内置演示录音(不依赖开发机上的 mock_device 进程);
3. **内置演示录音**:随包或由服务端下发一段演示音频,确保无硬件也能完成
   "同步 → 转写 → 摘要"完整路径;
4. **手机麦克风录音**:无硬件用户的兜底路径,必须可用(PRD §4 P0);
5. **操作说明**:审核备注里逐步写明上述路径,避免审核员认为"App 无法使用"。

第 2、3 项目前尚未实现(当前 Demo 依赖本地 mock_device 进程),
属于提交审核前的必做项,不在 Phase 3 范围内。
