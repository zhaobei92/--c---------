...
- **第二次 Ctrl+C 立即结束进程**（退出码 75）。第一次进入 3 秒宽限期；等不及的话再按一次，
  处理器直接 `_exit`。
- **未完整编译验证**。ROS-free 的部分已真实编译并运行通过：TF 安全策略 12/12、
  信号退出路径 5/5、Mock 签名对齐厂商头文件；依赖 rclcpp 的 .cpp 与集成测试尚未编译。
  详见 `docs/odin1/03-phase2-audit.md` §4。
